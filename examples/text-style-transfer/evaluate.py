...
def main(config: "DictConfig"):
    ...
    eval_dataloader = DataLoader(test_dataset,shuffle= True,batch_size= config.eval_config.batch_size,collate_fn=attack_collate_fn)
...
def evaluate_fn(prompt_model,prompt_model_tokenizer,reward_lm_fn,target_lm_fn,eval_dataloader,config):
    ...
    for batch in eval_dataloader:
        ...
        target_lm_generations = target_lm_fn(q_s,p_s,handler = None,mode = "infer")
        _reward_scores = reward_lm_fn(q_s,target_lm_generations)
        reward_scores = _reward_scores * config.trainer.reward_multiply
        reward_all.append(reward_scores)
        q_ss.extend(q_s)
        p_ss.extend(p_s)
        t_ss.extend(target_lm_generations)
    return torch.stack(reward_all).view(1,-1).cpu().tolist()[0],q_ss,p_ss,t_ss
